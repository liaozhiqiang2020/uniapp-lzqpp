		...
		methods: {
			...
			_confirm() {
				...
				this.treeList.forEach((v, i) => {
					if (this.treeList[i].checked) {
						...
						// 移除子元素
						delete obj.childTtAchievements
						rt.push(obj)
					}
				})
				this._hide()
			...
			_renderTreeList(list = [], rank = 0, parentId = [], parents = []) {
				list.forEach(item => {
					...
					if (Array.isArray(item.childTtAchievements) && item.childTtAchievements.length > 0) {
						// console.log(item)
						let parentid = [...parentId],
							parentArr = [...parents],
							childrenid = [...childrenid];
						delete parentArr.childTtAchievements
						parentid.push(item[this.idKey]);
						parentArr.push({
							[this.idKey]: item[this.idKey],
							[this.rangeKey]: item[this.rangeKey]
						})
						this._renderTreeList(item.childTtAchievements, rank + 1, parentid, parentArr);
					} else {
