<template>
	<view class="keyboard-main" :style="{height:height+'px'}">
		<text :style="{fontSize:parseInt((fontSize/3*2.2))+'px'}" style="margin-left: 30upx;">{{currency}}</text>
		<view class="keyboard-content" :style="{height:height+'px'}">
			<view class="placeholder" :style="{fontSize:parseInt((fontSize/3*2))+'px',height:height+'px'}"
				v-if="!money">
				{{placeholder}}
			</view>
			<view class="input" :class="{zIndex:isShow}"
				:style="{fontSize:size+'px',height:height+'px',fontWeight:isBold?'bold':'initial'}" @click.stop="show">
				<span id="text">
					<block v-if="isFilter">
						{{filterMoney(money)}}
					</block>
		...
		<view class='keyboard' :class="{noPadding:!isShow}">
			...
			<view class='key-confirm frist big' :class="'bg-' + themeColor.name"
